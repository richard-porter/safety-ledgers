...
ai_patterns = re.compile(r"^\s*(assistant|ai|claude|chatgpt|gpt|gemini|grok|deepseek)\s*[:\]]\s*", re.IGNORECASE)
...
r”(?|can you|could you|please|i want|i need|tell me|explain|what is|how do|why does|help me)”,
re.IGNORECASE
)
...
if not ai_turns:
return MetricResult(“Action Item Expansion Rate”, 0.0, “0 unsolicited expansions”, “healthy”, “No AI turns detected.”)
...
if not early_terms:
    return MetricResult("Frame Persistence", 1.0, "Could not extract terms", "watch", "Insufficient text for analysis.")
...
adopted = late_terms - early_terms
...
if extension_ratio <= 0.20 and ai_extensions <= 1:
    ...
    signal = "watch"
    note = f"Moderate post-completion extension ({post_completion_turns} turns after apparent task completion). {ai_extensions} AI engagement signal(s) detected."
else:
    signal = "alert"
    note = f"Significant post-completion extension ({post_completion_turns} turns, {extension_ratio:.0%} of session). {ai_extensions} AI engagement extension signal(s). Possible engagement optimization."
